...
#define TXRX_ENABLED								1	/* 1 = enable Tx/Rx in while loop. 0 = disable any Tx and Rx */
#define TRANSMIT_ONLY								1	/* 1 = set to transmit only. 0 = ping pong */
#define RECEIVE_ONLY								0	/* 1 = set to receive only. 0 = ping pong */
...
//SM
#define SM_CODE_ACTIVATE							0
#define SM2											0
#define MAX_PACKET_COUNT							3 //debug: make this smaller. default 10
#define SENSOR_READ_INTERVAL						1800000
#define PACKET_HEADER_SIZE							13
#define SENSOR_DATA_SIZE							20
#define TOTAL_PACKET_SIZE							(PACKET_HEADER_SIZE + SENSOR_DATA_SIZE)
//HAB
#define HAB_CODE_ACTIVATE							0
#define HAB2										0
#define SM_ID										2
#define CMD_ID										3
...
int main(void)
{
	...
	//Start up blink check
	//startUpFWCheck(&huart1, &hi2c1);
	blinkLED();
	HAL_Delay(2000);
	HAL_UART_Transmit(&huart1, (uint8_t*)"TRANSMIT MODE :\r\n", 17, HAL_MAX_DELAY);
...
void eventRxDone(pingPongFSM_t *const fsm)
{
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)"RX Received a packet\r\n", 22, HAL_MAX_DELAY);
...
void enterMasterRx(pingPongFSM_t *const fsm)
{
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)"Rx start\r\n", 17, HAL_MAX_DELAY);
...
void enterMasterTx(pingPongFSM_t *const fsm)
{
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)"Payload sent : ABCDEFGHIJKLMNOPQRST\r\n", 37, HAL_MAX_DELAY);
	HAL_Delay(2500); //mandatory delay? have to test
	switchTransmitOff();
}
